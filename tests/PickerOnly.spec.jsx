...
describe("DatePicker", () => {
  ...
  function renderRange({ value }) {
    return (
      <input
        ...
        value={
          value &&
          `${value[0] && value[0].format(format)} - ${value[1] &&
  ...
  it("close on ok", () => {
    ...
    picker.find(".rc-calendar-picker-input").simulate("click");
    expect(picker.find(".rc-calendar-ok-btn").debug()).not.toBe("");
    picker.find(".rc-calendar-ok-btn").simulate("click");
    expect(picker.find(".rc-calendar-ok-btn").debug()).toBe("");
  });

  it("close on clear", () => {
    const picker = renderPicker({ value: moment() });
    picker.find(".rc-calendar-picker-input").simulate("click");
    expect(picker.find(".rc-calendar-clear-btn").debug()).not.toBe("");
    picker.find(".rc-calendar-clear-btn").simulate("click");
    expect(picker.find(".rc-calendar-clear-btn").debug()).toBe("");
  });
